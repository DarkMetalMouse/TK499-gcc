...
void GPIO_Init(GPIO_TypeDef *GPIOx, GPIO_InitTypeDef *GPIO_InitStruct)
{
    ...
    if (GPIO_InitStruct->GPIO_Pin >> 16) // The pin is 16 to 23
    {
 ...
 *            @arg GPIO_AF_0: SPI1, MC0, TIM17_BKIN, SWDIO, SWCLK, UART1
 *            @arg GPIO_AF_1: UART1, TIM3_CH1, TIM3_CH2, TIM3_CH3, TIM3_CH4, I2C1
 *            @arg GPIO_AF_2: TIM2_CH1_ETR, TIM2_CH2, TIM2_CH3, TIM2_CH3,
 *                            TIM2_CH4, TIM1_BKIN, TIM1_CH1N, TIM1_CH1,
 *                            TIM1_CH2, TIM1_CH3, TIM1_CH4, TIM1_ETR,
 *                            TIM1_CH2N, TIM1_CH3N, TIM2_CH2, TIM1 6_BKIN,
 *                            TIM16_CH1N, TIM17_CH1N, TIM1 6_CH1, TIM17_CH1
...
void GPIO_PinAFConfig(GPIO_TypeDef *GPIOx, uint32_t GPIO_Pin, uint8_t GPIO_AF)
{
    ...
    if (GPIO_Pin >> 16) // The pin is 16 to 23
    {
        temp = GPIO_Pin >> 16;
        for (i = 0; i < 8; i++)
        {
            if (temp & 0x01)
            {
                GPIOE->AFRH_EXT &= ~((uint32_t)0xF << ((uint32_t)(i << 2))); // AF configuration occupies a nibble so << 2
                GPIOE->AFRH_EXT |= ((uint32_t)GPIO_AF << ((uint32_t)(i << 2)));
            }
            temp = temp >> 1;
        }
    }
    if (GPIO_Pin & 0XFF00) // The pin is 8 to 15
    {
        temp = GPIO_Pin >> 8;
        for (i = 0; i < 8; i++)
        {
            if (temp & 0x01)
            {
                GPIOx->AFRH &= ~((uint32_t)0xF << ((uint32_t)(i << 2))); // AF configuration occupies a nibble so << 2
                GPIOx->AFRH |= ((uint32_t)GPIO_AF << ((uint32_t)(i << 2)));
            }
            temp = temp >> 1;
        }
    }
    if (GPIO_Pin & 0XFF) // The pin is 0 to 7
    {
        temp = GPIO_Pin;
        for (i = 0; i < 8; i++)
        {
            if (temp & 0x01)
            {
                GPIOx->AFRL &= ~((uint32_t)0xF << ((uint32_t)(i << 2))); // AF configuration occupies a nibble so << 2
